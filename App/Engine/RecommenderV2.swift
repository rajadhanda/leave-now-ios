...
struct RecommendationResult {
    let best: LegacyRecommendation
    let fallback: LegacyRecommendation?
}
...
struct RecommenderV2 {
    ...
    func recommend(plans: [JourneyPlan], weather: Weather?, disruptions: [Disruption]) -> RecommendationResult? {
        ...
        func toRec(_ tup: (JourneyPlan, Int, Int, Double, Double)) -> LegacyRecommendation {
            LegacyRecommendation(plan: tup.0, p50Minutes: tup.1, p90Minutes: tup.2, confidence: tup.3, rationale: ExplanationBuilder().rationale(p50: tup.1, p90: tup.2, changes: tup.0.changes, rainDelta: 0, hasSevereDelaysOnKeyLeg: false, keyLineName: nil))
        }
